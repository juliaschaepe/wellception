...
echo "== GPU srun  =="
command="srun -p gpu -c 4 --gres gpu:1 --pty bash"
echo ${command}
ssh ${RESOURCE} ${command}

echo "== Running deepcell-tf container  =="
command="singularity shell -nv --pwd /scratch/pfordyce/deepcell-tf/ /scratch/pfordyce/deepcell-tf-img/deepcell-tf_latest.sif"
echo ${command}
ssh ${RESOURCE} ${command}
